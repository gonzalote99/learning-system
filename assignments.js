    ...
    function display(data) {
      ...
      data.forEach((elem, index) => {
        ...
        <div class="details">
         ...
         <p><b>${elem.instructor}</b> scheduled <b>assignment problem</b> at ${elem.schedule} - ${elem.time}</p>
        </div>
  ...
  if(value == 'All') {
    ...
    data.forEach((elem, index) => {
      ...
      <div class="details">
       <span>${elem.title}</span>
       <button class="type">${elem.type}</button>
       <p><b>${elem.instructor}</b>scheduled<b>${elem.category}</b> at ${elem.schedule} - ${elem.time}</p>
      </div>
      `;
      document.querySelector('.section').append(div);
    });
...
function filterTitle() {
  ...
  let Titlefilter = new lectureList();
  Titlefilter.filterByTitle(value);
}
